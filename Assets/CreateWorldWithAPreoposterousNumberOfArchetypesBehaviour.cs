...
{
    public class CreateWorldWithAPreoposterousNumberOfArchetypesBehaviour : MonoBehaviour
    {
        public enum API
        {
            ChunksWithoutGroups,
        ...
        private void Start()
        {
            ...
            Debug.Log($"API: {api}");
            finalOutput.AppendLine($"API: {api}");
            
            switch (api)
            {
                case API.ChunksWithoutGroups:
                    world.GetOrCreateManager<ChunksWithoutGroups>();
                    break;
                case API.ChunksWithGroups:
                    world.GetOrCreateManager<ChunksWithGroups>();
                    break;
                case API.JobsWithoutChunks:
                    world.GetOrCreateManager<JobsWithoutChunks>();
                    break;
                case API.ChunksAndJobs:
                    world.GetOrCreateManager<ChunksAndJobs>();
                    break;
                case API.DataArrays:
                    world.GetOrCreateManager<DataArrays>();
                    break;
            }
        ...
        private void Update()
        {
            ...
            if (DateTime.Now >= timeOfNextUpdate)
            {
                ...
                if (TargetArchetypeCount > 1000000)
                {
                    ...
                    System.IO.File.WriteAllText($"{api}.txt", finalOutput.ToString());
                    Application.Quit();
                }
            }
